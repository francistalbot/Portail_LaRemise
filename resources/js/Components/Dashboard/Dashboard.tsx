...
export const Dashboard = () => {
    ...
    const mockTimeSlots: TimeSlot[] = [
        {
            ...
            filledPositions: 2,
            positions: [
                { id: '1', name: 'Accueil', volunteer: 'Marie Dubois', isUrgent: false },
                { id: '2', name: 'Cuisine', volunteer: 'Jean Martin', isUrgent: false },
                { id: '3', name: 'Distribution', volunteer: '', isUrgent: true },
        ...
        },
        {
            ...
            filledPositions: 3,
            positions: [
                { id: '5', name: 'Accueil', volunteer: 'Sophie Tremblay', isUrgent: false },
                { id: '6', name: 'Distribution', volunteer: 'Pierre Lavoie', isUrgent: false },
                { id: '7', name: 'Accompagnement', volunteer: 'Claire Bouchard', isUrgent: false },
            ]
        },
        {
            id: '3',
            ...
            filledPositions: 1,
            positions: [
                { id: '8', name: 'Surveillance', volunteer: 'Marc Côté', isUrgent: false },
    ...
    // Calculs
    const totalPositions = mockTimeSlots.reduce((acc, slot) => acc + slot.totalPositions, 0);
    const totalFilled = mockTimeSlots.reduce((acc, slot) => acc + slot.filledPositions, 0);
    ...
    const vacantPositions = gridData.filter(item => item.Benevole === "VIDE").length;
    ...
    const getSlotStatus = (slot: TimeSlot) => {
        ...
        if (percentage === 100) return { color: 'bg-green-100 border-green-500', text: 'Complet' };
        if (percentage >= 50) return { color: 'bg-yellow-100 border-yellow-500', text: 'Partiel' };
        ...
        <div id="dashboard" className="p-6 bg-gray-50 min-h-screen">
                    ...
                    <div className="flex items-center">
                        <div className="ml-3">
                            <h3 className="text-sm font-medium text-red-800">
                                ⚠️ {urgentPositions + vacantPositions} postes urgents à combler
                            </h3>
                            <p className="text-xs text-red-700 mt-1">
                                Certains créneaux ne peuvent pas ouvrir sans ces postes.
                            </p>
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <div className="flex items-center">
                        <div className="flex-1">
                            <p className="text-sm font-medium text-gray-600">Couverture globale</p>
                            <p className={`text-2xl font-bold ${getCoverageColor(coveragePercentage)}`}>
                                {coveragePercentage}%
                            </p>
                        </div>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <div className="flex items-center">
                        <div className="flex-1">
                            <p className="text-sm font-medium text-gray-600">Postes comblés</p>
                            <p className="text-2xl font-bold text-green-600">{totalFilled}/{totalPositions}</p>
                        </div>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <div className="flex items-center">
                        <div className="flex-1">
                            <p className="text-sm font-medium text-gray-600">Postes urgents</p>
                            <p className="text-2xl font-bold text-red-600">{urgentPositions + vacantPositions}</p>
                        </div>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <div className="flex items-center">
                        <div className="flex-1">
                            <p className="text-sm font-medium text-gray-600">Bénévoles disponibles</p>
                            <p className="text-2xl font-bold text-blue-600">
                                {mockVolunteers.filter(v => v.isAvailable).length}
                            </p>
                        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white rounded-lg shadow-md">
                    <div className="p-6 border-b border-gray-200">
                        <div className="flex justify-between items-center">
                            <h2 className="text-xl font-semibold text-gray-900">Postes aujourd'hui</h2>
                            <Link 
                                to="/calendar" 
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Succursale</th>
                                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Comité</th>
                                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Poste</th>
                                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Bénévole</th>
                                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Action</th>
                                    </tr>
                                </thead>
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {gridData.map((item, index) => (
                                        <tr key={index} className={item.Benevole === "VIDE" ? "bg-red-50" : ""}>
                                            <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{item.Time}</td>
                                            <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{item.Succursal}</td>
                                            <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{item.Comite}</td>
                                            <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{item.Poste}</td>
                                            <td className="px-4 py-4 whitespace-nowrap text-sm">
                                                {item.Benevole === "VIDE" ? (
                                                    ...
                                                    <span className="text-gray-900">{item.Benevole}</span>
                                                )}
                                            </td>
                ...
                <div className="bg-white rounded-lg shadow-md">
                    <div className="p-6 border-b border-gray-200">
                        <h2 className="text-xl font-semibold text-gray-900">Bénévoles disponibles</h2>
                    </div>
                    <div className="p-6">
                                ...
                                <div key={volunteer.id} 
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex-1">
                                            ...
                                            <p className="text-xs text-gray-500">Actif il y a {volunteer.lastActivity}</p>
                                        </div>
                                        <div className="flex flex-col items-end gap-1">
                                            <span className={`w-3 h-3 rounded-full ${
                                                volunteer.isAvailable ? 'bg-green-500' : 'bg-gray-400'
                                            }`}></span>
                                            {volunteer.isAvailable && (
